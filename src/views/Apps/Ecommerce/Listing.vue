...
import NoResults from '../../../components/vito/NotFound/NoResults'
...
export default {
  ...
  methods: {
    ...
    checkCart (item) {
      let cartItem = this.stateCart.find(cart => cart.objectID === item.objectID)
      return cartItem !== undefined && cartItem !== null
    },
    wishCart (item) {
      let wishlistItem = this.stateWishlist.find(cart => cart.objectID === item.objectID)
